...
_checkAbandonded = {
	...
	if (_cleanThis) then{
		_nearEntities = (position _vehicle) nearEntities [["LandVehicle","Air","Ship","Man"], pvpfw_cleanUp_abandonRadius];
		if (({({alive _x} count crew _x) != 0} count _nearEntities) != 0) then{
			_cleanThis = false;
		};
	};
