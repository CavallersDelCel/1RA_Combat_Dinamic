...
if (_creatingtype == "random") then {
	...
	_newbase = [_foundSafePos, random 360, call (compile (preprocessFileLineNumbers "AOW_MissionCreator\Compositions\base.sqf"))] call BIS_fnc_ObjectsMapper;
...
if (_creatingtype == "manual") then {
	...
	["AOW_Relocate_mapclick","onMapSingleClick", {
...
["AOW_Relocate_mapclick", "onMapSingleClick"] call BIS_fnc_removeStackedEventHandler;

_newbase = [getMarkerPos "TempBase", _Bdirection, call (compile (preprocessFileLineNumbers "AOW_MissionCreator\Compositions\base.sqf"))] call BIS_fnc_ObjectsMapper;
    [] execVM "AOW_MissionCreator\BaseCreator\ReBuild.sqf";
};
...
if (player distance [0,0,0] < 100 ) then {player setPos [(getMarkerpos "respawn_west" select 0) + random 4 - random 4, (getMarkerpos "respawn_west" select 1) + random 4 - random 4,0];
	player setDir (markerDir "VVS1");
};

// Move player near the old base being deleted
{if (_x distance _oldbasepos < 100 && isPlayer _x) then {
	_x setPos [(getMarkerpos "respawn_west" select 0) + random 4 - random 4, (getMarkerpos "respawn_west" select 1) + random 4 - random 4,0]};
	_x setDir (markerDir "VVS1");
} forEach allUnits;

[localize "str_AOW_Game_Base5","hintSilent",nil,false] call BIS_fnc_MP;
