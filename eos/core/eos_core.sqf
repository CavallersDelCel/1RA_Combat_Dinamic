				...
				_units=_eosActivated getvariable _cacheGrp;
						...
						_aMin=_aSize select 0;
								...
								if (_aMin > 0) then {
									...
									if (_upsmon == 1 && _side != CIVILIAN) then {
										0 = [leader _aGroup,_mkr,"FORTIFY"] execVm "scripts\UPSMON.sqf";}
									else {0=[_mPos,units _aGroup,_mkrX,0,[0,20],true,true] call shk_fnc_fillhouse;};
												};};
		};
				...
				_units=_eosActivated getvariable _cacheGrp;
					...
					_bMin=_bSize select 0;
							...
							if (_bMin > 0) then {
								...
								if (_upsmon == 1 && _side != CIVILIAN) then {
									0 = [leader _bGroup,_mkr,"SAFE","LIMITED"] execVm "scripts\UPSMON.sqf";}
