...
waituntil {_vehicle distance _pad < 30 || !(alive _vehicle)};
if !(alive _vehicle) exitWith {deletevehicle _pad;};

		_grp leaveVehicle _vehicle;

waitUntil{sleep 1; {_x in _vehicle} count units _cargoGrp == 0 || !(alive _vehicle)};
if !(alive _vehicle) exitWith {deletevehicle _pad;};

		sleep 5;
		_vehicle flyInHeight 50;
		deletevehicle _pad;
						0 = [_grp,_mkr] call eos_fnc_taskpatrol;
