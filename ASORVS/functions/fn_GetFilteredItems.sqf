...
if(("autonomous" in ASORVS_VehicleTypes)) then {
	{
		if([_x select DBF_ClassName] call ASORVS_fnc_IsAllowed) then {
			_ret pushBack _x;
		};
	} forEach (ASORVS_DB select DB_Autonomous);
};
if(("support" in ASORVS_VehicleTypes)) then {
	{
		if([_x select DBF_ClassName] call ASORVS_fnc_IsAllowed) then {
			_ret pushBack _x;
		};
	} forEach (ASORVS_DB select DB_Support);
};
_ret